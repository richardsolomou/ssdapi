module.exports = function (app, passport, mysql, config) {
	...
	app.get(config.api.folder, function (req, res) {
		// Render index.ejs with any potential user credentials.
		return res.render('index', { name: 'index', user: req.session.user });
	});

	// Route for logging in to the API.
	app.get(config.api.folder + '/login', function (req, res) {
		// Create a variable with an error flash message.
		var error = req.flash('error') || [];
		// Use passport to check if the user is already authenticated.
		if (req.isAuthenticated() && req.session.user) return res.redirect(config.api.folder);
		// Check that there isn't an error.
		if (error.length == 0) return res.redirect(config.api.folder + '/auth/google');
		// Render login.ejs with any potential user credentials and error messages.
		return res.render('login', { user: req.session.user, error: error });
	});

	// Route to show the documentation.
	app.get(config.api.folder + '/documentation', function (req, res) {
		// Render documentation.ejs .
		return res.render('documentation', { name: 'documentation', user: req.session.user });
	});

	// Route to handle HTTP verbs for apps.
	app.route(config.api.folder + '/apps')
		// Matches all HTTP verbs for this route.
		.all(isLoggedIn)
		...
		.get(function (req, res) {
			...
			mysql.query('SELECT * FROM `api_apps` WHERE `user_id` = :user_id ORDER BY `created_at` DESC', { user_id: req.session.user.id }, function (err, apps) {
				...
				return res.render('apps', { name: 'apps', user: req.session.user, apps: apps });
			});
		})
		...
		// Route for deleting an existing application.
	...
	app.get(config.api.folder + '/logout', isLoggedIn, function (req, res) {
		// Use sessions to logout user.
		req.session.user = null;
		// Use passport to logout user.
		req.logout();
		// Redirect user to homepage.
		return res.redirect(config.api.folder);
	});

	// Route for google authentication.
	app.get(config.api.folder + '/auth/google', passport.authenticate('google', { approvalPrompt: 'force', loginHint: 'port.ac.uk' }));

	// Route for the google authentication callback.
	app.get(config.api.folder + '/auth/google/callback', passport.authenticate('google', {
		// Set redirect URL on successful log-in.
		successRedirect: config.api.folder,
		// Set redirect URL on failed log-in.
		failureRedirect: config.api.folder + '/login'
	}));

	// Set up error handling.
	app.get('*', function (req, res) {
		// Render index.ejs with any potential user credentials.
		return res.render('error', { name: 'error', user: req.session.user });
	});
	...
	function isLoggedIn(req, res, next) {
		...
		res.redirect(config.api.folder + '/auth/google');
	}
};